class Scrabble {
  ...
  score () {
    let scrabbleWordScore = 0
    let multiple = 1
    if (this.string === '' || this.string === null) {
      return 0
    } else if (this.string.includes('\t') || this.string.includes('\n') || this.string.includes(' ')) {
      return 0
    } else {
      for (let j = 0; j < this.string.length; j++) {
        if (this.string[j] === this.curlyOpen) {
          multiple = 2 * multiple
        }
        if (this.string[j] === this.squareOpen) {
          multiple = 3 * multiple
        }
        if (this.string[j] === this.curlyClosed) {
          multiple = multiple / 2
        }
        if (this.string[j] === this.squareClosed) {
          multiple = multiple / 3
        }
