...
// We have the word "street" so we need to slipt it down into letters. We want to check the alphabets. So we use .split() in an array.
  ...
  class Scrabble { //Created a class
  constructor (word) {
    this.word = word;
  }
  score () { //Function in the class is a method. This is a method.
    let score = 0 //Flexible variable
  if (typeof this.word !== "string") { //If it's NOT a string, return 0. This is a conditional flow with a strict inequality (!==)
    return 0;
  } else {
